<?php
	...
	function adjustment_inventory($product_id,$product_quantity){
		global $con;//Variable de conexion
		$sql=mysqli_query($con,"select * from inventory where product_id='".$product_id."'");//Consulta para verificar si el producto se encuentra registrado en  el inventario
		$count=mysqli_num_rows($sql);
		if ($count==0){
			$insert=mysqli_query($con,"insert into inventory (product_id, product_quantity) values ('$product_id','$product_quantity')");//Ingresa un nuevo producto al inventario
		} else {
	...
	//Guarda una venta
	function add_sale($sale_number, $customer_id, $sale_by,$sale_date){
		global $con;
		...
		(sale_id, sale_number, customer_id, sale_by, subtotal, tax, total, sale_date) 
		VALUES ('$sale_id', '$sale_number', '$customer_id', '$sale_by', '$total_neto', '$total_iva', '$total_venta', '$sale_date');";
	...
	//Guarda una compra
	function add_purchase($order_number, $supplier_id, $purchase_by,$purchase_date){
		global $con;
		...
		(purchase_id, purchase_order_number	, supplier_id, purchase_by, subtotal, tax, total, purchase_date) 
		VALUES ('$purchase_id', '$order_number', '$supplier_id', '$purchase_by', '$total_neto', '$total_iva', '$total_compra', '$purchase_date');";
		$query=mysqli_query($con,$sql);
		if ($query){
		 $true=1;
		} else {
